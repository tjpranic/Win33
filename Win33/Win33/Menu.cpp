...
Win33::Menu::Menu( ContextMenu* contextMenu, const std::wstring& text )
:
mHandle   ( CreateMenu( ) ),
mParent   ( Interop::toHandle( contextMenu ) ),
mPosition ( GetMenuItemCount( mParent ) )
{
    AppendMenu( mParent, MF_POPUP, reinterpret_cast<UINT_PTR>( mHandle ), text.c_str( ) );
}
Win33::Menu::Menu( MenuBar* menuBar, const std::wstring& text )
:
mHandle   ( CreateMenu( ) ),
mParent   ( Interop::toHandle( menuBar ) ),
mPosition ( GetMenuItemCount( mParent ) )
{
    AppendMenu( mParent, MF_POPUP, reinterpret_cast<UINT_PTR>( mHandle ), text.c_str( ) );
}
Win33::Menu::Menu( Menu* menu, const std::wstring& text )
:
mHandle   ( CreateMenu( ) ),
mParent   ( Interop::toHandle( menu ) ),
mPosition ( GetMenuItemCount( mParent ) )
{
    AppendMenu( mParent, MF_POPUP, reinterpret_cast<UINT_PTR>( mHandle ), text.c_str( ) );
}
